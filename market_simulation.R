...
distribute_wealth_pareto <- function(step_size = 0.1, supply = 1e9, n_traders = 500, loc = 1, disp = 3)
{

}

## FUNCTION: distributes shares of stock according to Pareto distribution
distribute_shares_pareto <- function(step_size = 0.1, n_shares = 10000, n_traders = 500, loc = 1, disp = 3)
{
	ownership_v <- rep(NA, n_traders)
	ownership_v[1] <- ppareto(step_size, loc, disp) * n_shares 
	ownership_v[2 : n_traders] <- n_shares * (ppareto(seq(from = 2 * step_size, by = step_size, length = n_traders - 1), loc, disp) -
	               ...
	               n_mom_traders = 1000, n_rand_traders = 1000)
{
	...
	gs_shrs_outs <- 10000 
	bac_shrs_outs <- 10000 
	swks_shrs_outs <- 10000 
	goos_shrs_outs <- 10000 
	ups_shrs_outs <- 10000 
	jpm_shrs_outs <- 10000 
